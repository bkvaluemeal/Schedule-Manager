...
/**
 * Schedule is a class that holds between 4 and 6 Classes.
 * Checks to make sure you are taking between 12-19 credits.
 * Uses an ArrayList to hold classes.
 * @author Josh Cohen
 * @version 1.00
 */
...
{   
    ...
    /**
     * intitalizes Schedule with an empty ArrayList 
     * of Class value
     */
    public Schedule()
    {
    ...
    public void addClass(Class scheduledClass) throws Exception
    {
        ...
        {
            if(schedule.get(i).getBeginningTime() == scheduledClass.getBeginningTime() 
                    && schedule.get(i).getDay().equals(scheduledClass.getDay()))
            {
                throw new Exception();
            }
            else if(schedule.get(i).getEndingTime() == scheduledClass.getEndingTime()
                    && schedule.get(i).getDay().equals(scheduledClass.getDay()))
            {
                throw new Exception();
            }
